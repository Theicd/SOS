...
var health=5;
// פילטר ריכוך לקלט ג'ויסטיק מובייל (עדינות תנועה)
var smoothedMoveX = 0;
var smoothedMoveY = 0;
...
function game_step(now) {
    ...
    // Mobile joystick input - תנועה וסיבוב מג'ויסטיק וירטואלי (עם ריכוך ועדינות)
    if (typeof mobileInput !== 'undefined') {
        var smoothFactor = 0.85; // 0=חד, 1=איטי
        smoothedMoveX = smoothedMoveX * smoothFactor + mobileInput.moveX * (1 - smoothFactor);
        smoothedMoveY = smoothedMoveY * smoothFactor + mobileInput.moveY * (1 - smoothFactor);
        
        // Deadzone קטנה
        if (Math.abs(smoothedMoveX) < 0.05) smoothedMoveX = 0;
        if (Math.abs(smoothedMoveY) < 0.05) smoothedMoveY = 0;

        // סיבוב שמאל/ימין (X axis של הג'ויסטיק)
        if (Math.abs(smoothedMoveX) > 0.08) {
            camera.theta += smoothedMoveX * dt / 220; // מעט יותר עדין
        }
        // תנועה קדימה/אחורה (Y axis של הג'ויסטיק) - למעלה = קדימה
        if (Math.abs(smoothedMoveY) > 0.08) {
            speed += (speed < 1) * 0.08; // האצה מתונה יותר
            move_dir.push(mat_vector_product(matrix_rotate_xy(-camera.theta), Y_DIR.scalar_multiply(dt / 16 * smoothedMoveY)));
        }
    }
    ...
    // ירי - גם מעכבר וגם מכפתור מובייל
    var isFiring = keys._ || (typeof mobileInput !== 'undefined' && mobileInput.firing);
    if (isFiring) {
        camera_shake += .1*(camera_shake < 1.2); // פחות זעזוע כדי למנוע היבהוב לבן
            ...
            var hits = detect_collision_positions(camera.position,
                                                  camera.position.add(
                                                      screen_forward_pos.scalar_multiply(1e4)).add(
                                                          urandom_vector().scalar_multiply(200*camera_shake)), true);
