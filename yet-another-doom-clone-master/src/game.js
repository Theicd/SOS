...
// Multiplayer: notify parent window when enemy is killed
function notifyEnemyKilled(enemyId, position) {
    if (window.parent && window.parent !== window) {
        ...
        // עדכון סטטיסטיקות מיידי כולל הריגות
        window.parent.postMessage({
            type: 'player_stats',
            health: health,
            maxHealth: 9,
            kills: killed_enemies,
            isCritical: health <= 2
        }, '*');
    }
}

// Mobile input state for virtual joystick/buttons
var mobileInput = { moveX: 0, moveY: 0, firing: false };

// Multiplayer: listen for remote enemy kills and mobile input
window.addEventListener('message', function(event) {
    if (!event.data) return;
    
    // טיפול בקלט מובייל מהג'ויסטיק
    if (event.data.type === 'mobile_input') {
        if (event.data.inputType === 'joystick') {
            mobileInput.moveX = event.data.x || 0; // סיבוב שמאל/ימין
            mobileInput.moveY = event.data.y || 0; // קדימה/אחורה
        } else if (event.data.inputType === 'fire') {
            mobileInput.firing = event.data.pressed || false;
        }
        return;
    }
    
    console.log('[DOOM MP] Message received:', event.data.type);
    
    if (event.data.type === 'remote_enemy_killed') {
        var enemyId = event.data.enemyId;
        var rx = event.data.x;
        var ry = event.data.y;
        console.log('[DOOM MP] Remote kill - ID:', enemyId, 'pos:', rx, ry);
        
        if (typeof objects === 'undefined') {
            console.log('[DOOM MP] Objects not ready');
            return;
        }
        ...
        for (var i = 0; i < objects.length; i++) {
            var obj = objects[i];
            if (obj instanceof BaseEnemy && !obj.dead && obj.uniqueId === enemyId) {
                targetEnemy = obj;
                console.log('[DOOM MP] Found enemy by ID:', enemyId);
                break;
            }
        }
        
        // If not found by ID, find closest by position
        if (!targetEnemy) {
            var closestDist = 100;
            for (var i = 0; i < objects.length; i++) {
                var obj = objects[i];
                if (obj instanceof BaseEnemy && !obj.dead) {
                    var dist = Math.sqrt(
                        Math.pow(obj.position.x - rx, 2) + 
                        Math.pow(obj.position.y - ry, 2)
                    );
                    if (dist < closestDist) {
                        closestDist = dist;
                        targetEnemy = obj;
            ...
            if (targetEnemy) {
                console.log('[DOOM MP] Found enemy by position, dist:', closestDist);
            }
        }
        
        // Kill the enemy
        if (targetEnemy) {
            console.log('[DOOM MP] Killing enemy!');
            targetEnemy.dead = true;
            killed_enemies++;
            
            // Visual effects
            range(30).map(function() { 
                objects.push(new Hit(targetEnemy.position, 4)); 
            });
            play(sounds.boom);
            
            // Body parts
            if (targetEnemy.components) {
                targetEnemy.components.map(function(x) {
                    objects.push(new BodyPart(x, targetEnemy.position.add(x.position),
                        mat_vector_product(matrix_rotate_xy(-camera.theta),
                            NewVector(urandom(), 3+urandom(), 1+Math.random()))));
                });
            }
        } else {
            console.log('[DOOM MP] No enemy found to kill');
        }
    }
});
...
class BaseEnemy {
    constructor(position, theta, height, size) {
        ...
        this.uniqueId = ++enemy_id_counter;
    ...
    onhit(other) {
        ...
        // Multiplayer: notify about this kill
        notifyEnemyKilled(this.uniqueId, this.position);
...
function update_health(amt) {
    ...
    // שידור נתוני שחקן לחלון האב עבור HUD חיצוני
    if (window.parent && window.parent !== window) {
        window.parent.postMessage({
            type: 'player_stats',
            health: health,
