...
;(function initTrivia(window, document) {
  ...
  // חלק בנק שאלות – 10 שאלות פשוטות לבדיקת המערכת
  const QUESTIONS = [
  ...
  // חלק מצב – שומר הפניות ל-UI ולמצב המשחק
  const state = {
    ui: {},
    seeking: false,
    roomId: null,
  ...
  // חלק UI – טוען stylesheet ובונה את הכפתור והמודאל
  function buildUI() {
  ...
  // חלק פרסום – הודעות משחק (שאלות/תשובות)
  ...
  // חלק אירועים – טיפול בהודעות סטטוס
  ...
  // חלק אירועים – טיפול בהודעות משחק
  function onMatchEvent(evt) {
    ...
    switch (payload.type) {
      ...
      case 'question':
        ...
        renderQuestion(payload.questionIndex, payload.round || 0, payload.totalRounds || CFG.MAX_ROUNDS, payload.startedAt || Math.floor(Date.now() / 1000), payload.timeLimit || CFG.QUESTION_TIME);
        break;
      case 'answer':
  ...
  // חלק אירועים – הגנה מפני כפילויות
  ...
  // חלק רישום – הרשמה לאירועי Nostr מרגע שה-pool זמין
  function subscribe() {
  ...
  // חלק רישום – ביטול רישומים קיימים
  ...
  // חלק bootstrap – חיבור ל-notifyPoolReady כך שהמודול יופעל לאחר טעינת ה-pool
  ...
  // חלק bootstrap – הפעלת המודול לאחר טעינת ה-DOM
  function bootstrap() {
    buildUI();
    hookPoolReady();
    if (App.pool) {
