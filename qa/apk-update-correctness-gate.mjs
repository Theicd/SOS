...
record('apk-version url points to 1.0.124', String(apkMeta.url || '').includes('downloads/SOS-1.0.124.apk'));
record('apk-version advertises 1.0.124 / 125', apkMeta.version === '1.0.124' && Number(apkMeta.versionCode) === 125);
...
record('PUBLIC STABLE is 1.0.124 for initial install',
  /const NATIVE_APK_VERSION = '1\.0\.124'/.test(installerSrc));
record('apk-version published with matching 1.0.124 URL',
  apkMeta.published === true
  && String(apkMeta.url).includes('downloads/SOS-1.0.124.apk')
  && String(apkMeta.file) === 'SOS-1.0.124.apk');
...
record('BuildConfig shell versionName 1.0.124', /versionName\s*=\s*"1\.0\.124"/.test(gradle));
record('BuildConfig shell versionCode 125', /versionCode\s*=\s*125/.test(gradle));
record('published apk-version is 1.0.124 / 125',
...
const V122_URL = 'https://sos010.com/downloads/SOS-1.0.124.apk';
const V123_URL = 'https://sos010.com/downloads/SOS-1.0.124.apk';
...
{
  const r = decideUpdate('1.0.124', 124, {
    version: '1.0.124',
    versionCode: 124,
    file: 'SOS-1.0.124.apk',
    url: V122_URL,
    published: true,
  });
...
{
  const r = decideUpdate('1.0.123', 124, {
    version: '1.0.124',
    versionCode: 125,
    file: 'SOS-1.0.124.apk',
    url: V122_URL,
  ...
  record('CASE E toast offers 1.0.124', r.toast === true && r.download === true);
  record('CASE E exact 1.0.124 URL used', r.url === V122_URL);
