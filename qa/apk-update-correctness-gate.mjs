...
record('apk-version url points to 1.0.119', String(apkMeta.url || '').includes('apk-1.0.119/SOS-1.0.119.apk'));
record('apk-version advertises 1.0.119 / 120', apkMeta.version === '1.0.119' && Number(apkMeta.versionCode) === 120);
...
record('PUBLIC STABLE is 1.0.119 for initial install',
  /const NATIVE_APK_VERSION = '1\.0\.119'/.test(installerSrc));
record('apk-version published with matching 1.0.119 URL',
  apkMeta.published === true
  && String(apkMeta.url).includes('apk-1.0.119')
  && String(apkMeta.file) === 'SOS-1.0.119.apk');
...
record('published apk-version is 1.0.119 / 120',
...

{
  ...
  record('CASE E toast offers 1.0.119', r.toast === true && r.download === true);
  record('CASE E exact 1.0.119 URL used', r.url === V119_URL);
...

{
  ...
  record('future newer release uses exact URL', r.toast === true && r.url === V120_URL);
}
